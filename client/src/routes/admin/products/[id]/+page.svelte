    ...
    onMount(async () => {
        try {
            ...
            const productResponse = await fetch(
                `${GLOBAL.SERVER_URL}/products/${$page.params.id}?userId=${$auth?.user?.id}`,
                {
                    headers: {
            ...
            if (!productData.categoryId) {
                console.warn('Product data missing categoryId:', productData);
            ...
            const categoriesResponse = await fetch(
                `${GLOBAL.SERVER_URL}/categories?userId=${$auth?.user?.id}`,
                {
                    headers: {
                        Authorization: `Bearer ${$auth.token}`
                    }
            ...
            error = `An error occurred while fetching product data: ${err.message}`;
            console.error(err);
        } finally {
            loading = false;
        }
    ...
    async function updateProduct(): Promise<void> {
        ...
        try {
            const response = await fetch(
                `${GLOBAL.SERVER_URL}/admin/products/${data.id}?userId=${$auth?.user?.id}`,
                {
                    method: 'PATCH',
                    headers: {
                    ...
                    body: JSON.stringify({
                        ...
                        specs: product.specs.filter((spec) => spec.key && spec.value)
                    })
                }
            );
            ...
            if (response.ok) {
                ...
                success = data.message || 'Product updated successfully';
                setTimeout(() => goto('/admin'), 2000); // Redirect after success
            } else {
                const data = await response.json();
            ...
            <form on:submit|preventDefault={updateProduct} class="space-y-6">
                ...
                >
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        ...
                        <div>
                            <label class="font-pixel text-retroGray block mb-2"

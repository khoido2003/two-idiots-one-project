		...
		{#if $cart.length > 0}
			...
			<div class="space-y-6 mb-8">
				{#each $cart as item (item.id)}
					...
					>
						<div class="grid grid-cols-1 sm:grid-cols-[auto_1fr_auto] gap-4 sm:gap-6 items-center">
							<!-- Image -->
							<div
							...
							<div class="flex flex-col justify-center">
								<h2 class="font-pixel text-retroGray text-xl sm:text-2xl leading-tight">
									{item.name}
								</h2>
							...
							>
								<div class="flex items-center justify-center gap-6">
									...
									<span class="font-pixel text-retroGray text-lg min-w-[2rem] text-center font-bold"
										>{item.quantity}</span
									>
									<button
										on:click={() => updateQuantity(item.id, 1)}
								...
								<button
									...
									class="text-retroGray hover:text-retroCoral font-pixel text-sm sm:text-base transition-colors underline"
								>
									Remove
								</button>
							</div>
